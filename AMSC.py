...
def resumen_estacion(Estacion,path_in3,columnas,path_out1):
    ...
    for i in tqdm(range(len(C1))):
        ...
        if estacion==Estacion:# Filtro para seleccionar la estación
            ...
            #Variables
            v["Ran_Rate"]=v['Rain_Rate']*0.2/60. #in units of cm/hr
        ...
        #######################################
            ...
            try:
                v1['fecha'] = pd.to_datetime(
                    v1['fecha']) - pd.to_timedelta(5, unit='h')
                ...
                v1['fecha'] = v1['fecha'] - pd.to_timedelta(5, unit='h')
                end_time = time.time()
                execution_time = end_time - start_time
                print("\n fecha inicial: ", v1.fecha.min(),
            ...
            #Conversiones
            #v1["Ran_Rate"] = v1['Rain_Rate']*0.2/60.  # in units cm/hr
            #if Estacion in ["caucasia", "santafe"]:
            #    v1['Bar'] = (v1['Bar']/1000.*(3386.389/100.0))
            #else:
            #    v1['Bar'] = (v1['Bar']/1000.*(3386.389/100.0))
            
            #v1['Out_Temp'] = ( v1['Out_Temp']/10. - 32.) * (5.0/9.0)
            #v1.loc[(v1['Out_Temp'] > 50) | (v1['Out_Temp'] < -15)] = np.nan
        ...
        if estacion==Estacion:
            ...
            for ii in tqdm(range(len(C12))):
                ...
                for iii in range(len(C13)):
                    try:
                        # Aquí colocas el código que pueda generar el error
                        # Por ejemplo, intentaremos leer un archivo CSV vacío
